...
#-# from img_array_transform_jh import ArrayTransform

cv2.setNumThreads(0)

RATIO=(1511/1516) # Seitenverhältnis der Originalbilder Weite/Höhe
# SIZE=(512, 512)   # Größe der Eingangsbilder/Masken und der Ergebnismasken

class Crag_DataLoader(object):
    '''
    Lädt Dataset CRAG-v2
    ratio = (1511 / 1516 - Seitenverhältnis der Originalbilder
    
    '''
    #+ def __init__(self,filenames_inputs, filenames_targets, patch_size,  dataset_repeat=1, images=np.arange(0, 70), validation=False):
    def __init__(self,filenames_inputs, filenames_targets, patch_size, dataset_repeat=1, images=np.arange(0, 70), validation=False):    
        self.image_filenames = filenames_inputs #  'Warwick QU Dataset (Released 2016_07_08)/train_'
        self.mask_filenames= filenames_targets
    ...
    def index_to_filename(self, index):
        ...
        image = self.image_filenames[index_img] #+ '.png'
        mask  = self.mask_filenames[index_img]  #+ '.png'
        return image, mask
    ...
    def open_and_resize(self, image, mask):
        ...
        # Seitenverhältnis der Originalbilder
        #ratio = (1511 / 1516) #+ CRAG_v2
        ratio = RATIO #+ CRAG_v2
        #?ratio = (self.w / self.h) #+
