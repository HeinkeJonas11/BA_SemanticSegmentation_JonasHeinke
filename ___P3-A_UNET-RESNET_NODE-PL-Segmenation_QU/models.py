...
class LevelBlock(nn.Module):
    def __init__(self, depth, total_depth, in_channels, out_channels):
        ...
        if depth > 1:
            self.encode = ConvBlock(in_channels, out_channels) ## self.encode   Vererbte Variable aus nn.Module? Oder selbst
            self.down = nn.MaxPool2d(2, 2)
            self.next = LevelBlock(depth - 1, total_depth, out_channels, out_channels * 2)
            next_out = list(self.next.modules())[-2].out_channels    ## Klassenvariable geändert, nicht Instance/Objektvariable?   -- next_out Vorbereitung für den Aufwertszweig -- Levelblock in der Netzwerkausgabe
            self.up = nn.ConvTranspose2d(next_out, next_out // 2, 2, 2)   ## Aufwertszweig
